        ...
        for (ref attr, &(ref counter, _)) in &self.map {
            if *counter.borrow() == 0 {
                match field {
                    Some(field) => {
                        println!(
                            "stomp-macros: unexpected attribute '{}' on field '{}' of struct '{}'",
                            attr,
                            field,
                            name
                        )
                    ...
                    None => {
                        println!(
                            "stomp-macros: unexpected attribute '{}' on struct '{}'",
                            attr,
                            name
                        )
                    }
...
impl FieldAttributes {
    pub fn check_used(&self, name: &str) {
        for (ref field, &(ref counter, ref attrs)) in &self.map {
            if *counter.borrow() == 0 {
                panic!(
                    "stomp-macros: didn't access attributes for field '{}' on struct '{}' for some reason",
                    field,
                    name
                );
            }
...
fn extract_attrs_inner(attrs: &mut Vec<syn::Attribute>) -> Attributes {
    ...
    attrs.retain(|attr| if let syn::MetaItem::List(ref ident, ref values) =
        attr.value
    {
        if ident == "stomp" {
            for value in values {
                    ...
                    syn::NestedMetaItem::MetaItem(ref item) => {
                        match *item {
                            syn::MetaItem::NameValue(ref name, ref value) => {
                                let &mut (_, ref mut attr) = stomps.entry(name.to_string()).or_insert((
                                    RefCell::new(0),
                                    Attribute::new(
                                        name.to_string(),
                                    ),
                            ...
                            syn::MetaItem::List(ref ident, ref values) => {
                                let &mut (_, ref mut attr) = stomps.entry(ident.as_ref().to_string()).or_insert((
                                    RefCell::new(0),
                                ...
                                for value in values {
                                    match *value {
                                        syn::NestedMetaItem::MetaItem(ref item) => {
                                            match *item {
                                                ...
                                                syn::MetaItem::NameValue(..) => {
                                                    panic!(
                                                        "Invalid stomp attribute {} named value in sublist not supported",
                                                        "hacked"
                                                    );
                                                }
                                                syn::MetaItem::List(..) => {
                                                    panic!(
                                                        "Invalid stomp attribute {} sublist in sublist not supported",
                                                        "hacked"
                                                    );
                                                }
                                            }
                                        ...
                                        syn::NestedMetaItem::Literal(_) => {
                                            panic!(
                                                "Invalid stomp attribute {} literal value not supported",
                                                "hacked"
                                            );
                                        }
                                    }
    ...
    let docs = attrs.iter()
        ...
        _ => unreachable!(),
                    ...
                    .fold(String::new(), |docs, line| {
        docs + line.trim_left_matches('/').trim() + "\n"
    });
...
pub fn extract_attrs(ast: &mut syn::MacroInput) -> (Attributes, FieldAttributes) {
    ...
    let field_attrs = match ast.body {
        ...
        syn::Body::Struct(syn::VariantData::Tuple(_)) => panic!("TODO: tuple struct unsupported msg"),
        syn::Body::Struct(syn::VariantData::Unit) |
        syn::Body::Enum(_) => HashMap::new(),
    };
    (
        root_attrs,
        FieldAttributes {
